...
// Get attendance data for all employees by month/year (Admin only)
router.get("/all/:month/:year", async (req, res) => {
  const { month, year } = req.params;

  try {
    console.log('All employees attendance request received:', { month, year });
    ...
    }).populate('employeeId', 'name department position email');
    ...
    allAttendanceRecords.forEach(record => {
      ...
      attendanceByEmployee[empId].records.push(record);
    });
    
    // Format the data for the frontend
    const result = employees.map(employee => {
      const empData = attendanceByEmployee[employee._id.toString()] || { employee, records: [] };
      ...
      const attendanceRecords = empData.records.map(record => {
        let status = 'absent';
        let hoursWorked = 0;
        ...
        return {
          date: record.date,
          status,
          inTime: record.inTime ? new Date(record.inTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : '',
          outTime: record.outTime ? new Date(record.outTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : '',
          hoursWorked: hoursWorked.toFixed(1)
        };
      });
      
      const avgHours = presentDays > 0 ? (totalHours / presentDays).toFixed(1) : '0.0';
      const workingDays = endDate.getDate() - 8; // Approximate working days (excluding weekends)
    ...
    console.error('Error in GET /all/:month/:year:', error);
    res.status(500).json({ error: error.message });
  }
});
